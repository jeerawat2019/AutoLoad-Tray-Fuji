...
namespace Ai_PCSystem.Strings
{
    public class Str_Enum
    {
        ...
        public class StringEnum
        {
            ...
            /// <returns></returns>
            public static List<string> GetArrayEnum<T>(string key, T defaults)
            {
                string data;
                List<string> ArrayEnum = new List<string>();
                for (int i = 0; i < (int)((T[])Enum.GetValues(typeof(T))).Length; i++)
                {
                    data = Convert.ToString((T)((T[])Enum.GetValues(typeof(T)))[i]);
                    ArrayEnum.Add(data.Substring(key.Length, 3));
                }
                return ArrayEnum;
            }
            #endregion
